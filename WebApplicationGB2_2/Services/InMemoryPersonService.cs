...
namespace WebApplicationGB2_2.Services
{
    public class InMemoryPersonService : IPersonService
    {
        ...
        public void Delete(int id)
        {
            try
            {
                _data.Remove(_data.First(p => p.Id == id));
            }
            catch (Exception e)
            {
        ...
        {
            return _data.FirstOrDefault(p => p.FirstName == name);
        }
        ...
        public void Update(Person person)
        {
            try
            {
                var el = _data.First(p => p.Id == person.Id);
                el.Age = person.Age;
                el.FirstName = person.FirstName;
                el.LastName = person.LastName;
                el.Email = person.Email;
                el.Company = person.Company;
            }
            catch (Exception e)
            {
